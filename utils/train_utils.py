...
def model_lc(opt: options.Options, override_model: Optional[str] = None) -> Tuple[models_utils.Model, options.Options]:
    ...
    def save_model(model_: models_utils.Model, suffix: str = '', optimizer=None, scheduler=None, epoch=None):
    ...
    model, optimizer_dict, scheduler_dict, epoch = load_model(opt, opt.device, suffix=override_model)
    model.save_model = save_model
    return model, opt, optimizer_dict, scheduler_dict, epoch
...
class Logger:
    ...
    def start(self, iters: int, tag: str = '', epoch: int = 0):
