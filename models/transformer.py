...
class CrossTransformerLayer(TransformerLayer):
    ...
    def forward_attention(self,x, y: Optional[T] = None, mask: Optional[T] = None, alpha: TN = None):
        x = x + self.attn0(self.norm0(x), mask)[0]
        out, attention = super(CrossTransformerLayer, self).forward_with_attention(x, y)
        return out, attention
...
class CombTransformer(nn.Module):
    ...
    def forward_attention(self, x, y: TN = None, mask: TN = None):
        attention = []
        for layer in self.layers:
            x, attention_ = layer.forward_attention(x, y, mask)
...
class CombLlavaTransformer(nn.Module):
    ...
    def forward(self, x, y: TN = None, l_feat=None, mask: TN = None):
        for li in range(len(self.layers)):
            x = self.layers[li](x, y, mask)
            x = self.l_layers[li](x, l_feat, mask)
        return x

    def __init__(self, dim_self: int, dim_llava: int, num_heads: int, num_layers: int, dim_ref: int,
                 mlp_ratio: float = 2., act=nnf.relu, norm_layer: nn.Module = nn.LayerNorm):
        super(CombLlavaTransformer, self).__init__()
        self.layers = nn.ModuleList([CrossTransformerLayer(dim_self, dim_ref, num_heads, mlp_ratio, act=act,
                                                           norm_layer=norm_layer) for _ in range(num_layers)])
        
        self.l_layers = nn.ModuleList([CrossTransformerLayer(dim_self, dim_llava, num_heads, mlp_ratio, act=act,
                                                           norm_layer=norm_layer) for _ in range(num_layers)])
