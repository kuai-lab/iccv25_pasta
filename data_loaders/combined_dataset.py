...
class CombinedDs(Dataset):
    def __getitem__(self, item: int):
        if item < self.len_sketch_ds :
            if self.part_learning:
                image_full, image_masked, masks, zh, l_feat, dist_adj, part_dist_adj = self.sketch_ds[item]
                weight_masked = torch.ones((1, 1))
                return weight_masked, image_full, image_masked, masks, zh, l_feat, dist_adj, part_dist_adj
            else:
                image_full, zh, l_feat, dist_adj, part_dist_adj = self.sketch_ds[item]
            ...
            image_full, zh, l_feat, dist_adj, part_dist_adj = self.clipasso_ds[idx]
            image_masked = image_full
            masks = torch.ones((16, 1))
            weight_masked = torch.zeros((1, 1))
            return weight_masked, image_full, image_masked, masks, zh, l_feat, dist_adj, part_dist_adj
            ...
            image_full, zh, l_feat, dist_adj, part_dist_adj = self.prosketch_ds[idx]
            image_masked = image_full
            masks = torch.ones((16, 1))
            weight_masked = torch.zeros((1, 1))
            return weight_masked, image_full, image_masked, masks, zh, l_feat, dist_adj, part_dist_adj
    ...
    def __init__(self, opt_skds: options.SketchOptions, opt_clds: options.SketchOptions, opt_prosk: Optional[options.SketchOptions]):
        self.part_learning = opt_skds.part_learning
