...
class SketchDs(Dataset):
    ...
    def load_item(self, item: int, select: Optional[int] = None) -> Tuple[TS, ...]:
        ...
        max_token_len = 53
        obj_name = self.paths[item][1]
        view = select
        llava_feat_path = f'{self.llava_base_dir}/view_{view}/sketch_{int(obj_name):06d}.npy'
        ...
        return self.split_by_view(raw_data, select), l_feat, obj_name, view

    def load_img(self, item: int) -> Tuple[TS, ...]:
        view = (int) (self.get_random() * 6)
        path = self.img_paths[view][item]
        ...
        try:
            ...
            l_feat = torch.zeros([max_token_len, 4096])
        return image, filename, id, view, l_feat
    ...
    def __getitem__(self, item: int):
        if self.part_learning:
            (points, depth_maps, masks, supports), l_feat, obj_name, view = self.load_item(item)
            ...
            return image_full, image, part_select.float().unsqueeze(-1), zh, l_feat, dist_adj
        else:
            image, filename, id, view, l_feat = self.load_img(item)
            zh = self.get_zh(id)
            dist_adj = self.get_adj(id)
            part_dist_adj = self.get_part_adj(id)
            # set zero for part_dist_adj
            # part_dist_adj = torch.zeros(16, 16)
    ...
    def __init__(self, opt: options.SketchOptions):
        self.opt = opt
        self.part_learning = opt.part_learning
        self.res = 256
        out_root = f'{constants.DATA_ROOT}/dataset_chair_preprocess/{opt.spaghetti_tag}/'
        self.paths = files_utils.collect(out_root, '.pkl')

        data_paths = [f'{constants.DATA_ROOT}/dataset_chair_preprocess/npr/view_{i}/' for i in range(6)]
        self.img_paths = [files_utils.collect(data_path, '.png') for data_path in data_paths]
        ...
        print("Loaded dist adj: ", self.dist_adj.shape, "in NPR")
        ...
        print("Loaded dist adj: ", self.part_dist_adj.shape, "in NPR")
